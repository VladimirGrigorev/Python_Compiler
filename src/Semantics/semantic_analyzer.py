...
import src.Сompiler.custom_builtins
from src.Semantics.semantic_components import *
from src.AST.Nodes import *

builtin_funcs = [f for f in dir(src.Сompiler.custom_builtins) if inspect.isfunction(getattr(src.Сompiler.custom_builtins, f))]
    ...
    def __init__(self):
        ...
        for func in builtin_funcs:
            self.__current_scope.add_label(Label(LabelType.FUNC, str(func),
                                                 len(inspect.signature(getattr(src.Сompiler.custom_builtins, func)).parameters)), [])
        self.errors = []
